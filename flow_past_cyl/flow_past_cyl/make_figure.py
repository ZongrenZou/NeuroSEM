...
if __name__ == "__main__":
    N_f = 100   # to Skip Points
    N_T = 100
    xyt_min = jnp.array([-3.9923, -3.8717, 0.0])
    xyt_max = jnp.array([11.813, 3.8717, 19.8])
    ...
    pinn = eqx.tree_deserialise_leaves("./checkpoints/RBC_4k_3555_40.eqx", pinn)
    ...
    # start_id_test = 70 * N_data
    # end_id_test = 71 * N_data
    start_id_test = 0
    end_id_test = -1
    x_star = jnp.array(xyt_data[:, 0])
    y_star = jnp.array(xyt_data[:, 1])
    t_star = jnp.array(xyt_data[:, 2])
    theta_star = jnp.array(theta_data)
    ...
    start_id_test = 99 * N_data
    end_id_test = 100 * N_data
