...
# @jax.jit
def loss_fn(network, weight_d, weight_f, ds_f, ds_bc, ds_ic, ds_cyl, ds_d):
    print("Number of data for initial condition: ", ds_ic["xyt_ic"].shape)
    print("Number of data for boundary condition: ", ds_bc["xyt_bc"].shape)
    print("Number of data for the cylinder: ", ds_cyl["xyt_cyl"].shape)
    ...
    theta_d_nn = jax.vmap(network, in_axes=(0, 0, 0))(ds_d["xyt_d"][:, 0], ds_d["xyt_d"][:, 1],
                                                       ds_d["xyt_d"][:, 2])
    ...
    theta_d_nn = theta_d_nn.flatten()[:, None]
    theta_cyl_nn = theta_cyl_nn.flatten()[:, None]

    theta_ic_d = ds_ic["theta_ic"]
    theta_bc_d = ds_bc["theta_bc"]
    theta_d = ds_d["theta_d"]
    theta_cyl_d = ds_cyl["theta_cyl"]
    
    loss_ic = jnp.mean(jnp.square(theta_ic_nn - theta_ic_d))
    loss_bc = 0 * jnp.mean(jnp.square(theta_bc_nn - theta_bc_d)) + jnp.mean(jnp.square(theta_cyl_nn - theta_cyl_d))
    loss_d = jnp.mean(jnp.square(theta_d_nn - theta_d))
    ...
    total_loss = weight_d * loss_ic + weight_d * loss_bc + weight_d * loss_d + loss_f
...
if __name__ == "__main__":
    ...
    for i in range(t[:N_T].shape[0]):
        idx = np.random.choice(N, N, replace=False)[:K]
        x_d += [x_data[idx].reshape([-1, 1])]
        y_d += [y_data[idx].reshape([-1, 1])]
        t_d += [t[i] * np.ones_like(x_d[-1])]
        theta_d += [theta_data[i, idx].reshape([-1, 1])]
    # stack them
    x_d = np.concatenate(x_d, axis=-1)
    y_d = np.concatenate(y_d, axis=-1)
    t_d = np.concatenate(t_d, axis=-1)
    theta_d = np.concatenate(theta_d, axis=-1)
    ...
    print("Measurements of u and v")
    print(u_f.shape, v_f.shape, x_f.shape, y_f.shape, t_f.shape, xyt_f.shape) 
    print("Scale of u:")
    print(np.max(np.abs(u_f)), np.mean(np.abs(u_f)))
    print("Scale of v:")
    print(np.max(np.abs(v_f)), np.mean(np.abs(v_f)))
    noise_u = 0.01
    noise_v = 0.01
    u_f = u_f + noise_u * np.random.normal(size=u_f.shape)
    v_f = v_f + noise_v * np.random.normal(size=v_f.shape)
    data_f = {
        "xyt_f": xyt_f, 
        "u_f": u_f, 
        "v_f": v_f,
    ...
    x_d = x_d.flatten().reshape([-1, 1])
    y_d = y_d.flatten().reshape([-1, 1])
    t_d = t_d.flatten().reshape([-1, 1])
    xyt_d = np.concatenate([x_d, y_d, t_d], axis=-1)
    theta_d = theta_d.flatten().reshape([-1, 1])
    print("Measurements of theta")
    print(theta_d.shape, xyt_d.shape)  
    print("Scale of theta:")
    print(np.max(np.abs(theta_d)), np.mean(np.abs(theta_d)))
    noise_theta = 0.01
    ...
    def train_step_opt(network, params_state):
        l, grad = eqx.filter_value_and_grad(loss_fn)(network, DATA_WEIGHT, RESIDUAL_WEIGHT,
                                                     data_f, data_bc, data_ic, data_cyl, data_d)
    ...
    # save model
    SAVE_MODEL_NAME = "./checkpoints/RBC_T_5_new_case_5kuv_5ktheta2.eqx"
