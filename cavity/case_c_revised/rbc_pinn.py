...
# case 0: 0.01 for uv, 0.01 for theta, 0 for weight decay
# case 1: 0.01 for uv, 0.05 for theta, 1e-1 for weight decay
# case 2: 0.05 for uv, 0.01 for theta, 1e-1 for weight decay
# case 3: 0.01 for uv, 0.025 for theta, 1e-1 for weight decay
# case 4: 0.025 for uv, 0.01 for theta, 1e-1 for weight decay
noise_uv = 0.025
noise_theta = 0.01
weight_decay = 1e-1


MODEL_FILE_NAME = "./checkpoints/RBC_1e4_case4.eqx"
DATA_FILE_NAME = "../case_b/data/data_1e4.mat"
SAVE_DATA_FILE_NAME = "./outputs/case_c_case4.mat"
...
if __name__ == "__main__":
    ...
    print("Scales of data (u, v, theta):")
    print(np.max(np.abs(u)), np.max(np.abs(v)), np.max(np.abs(theta)))
    print(np.mean(np.abs(u)), np.mean(np.abs(v)), np.mean(np.abs(theta)))
    ...
    ############# The values of data  ##############
    ################################################
    print(u_res.shape, v_res.shape, theta_d.shape)
    print(np.mean(np.abs(u_res)), np.mean(np.abs(v_res)), np.mean(np.abs(theta_d)))
