...
SAVE_FILE_NAME = "./outputs/RBC_1e4_noisy_data.mat"
...
if __name__ == "__main__":
    ...
    # add noise
    u_res += noise_uv * np.random.normal(size=u_res.shape)
    v_res += noise_uv * np.random.normal(size=v_res.shape)
    theta_tb += noise_theta * np.random.normal(size=theta_tb.shape)
    theta_bb += noise_theta * np.random.normal(size=theta_bb.shape)
